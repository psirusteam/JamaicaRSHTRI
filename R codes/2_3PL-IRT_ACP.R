...
dat_1 <- readRDS("Data/RHS2021_HH_trimmed.rds")
dat_2 <- dat_1 %>%
...
data_homal <- homals(dat_2, ndim = 2, level = "nominal")
...
dat_1 %>% select(matches("score")) %>% 
  mutate(score_acp_R = Quantil_acp1,
         score_homals_R = Quantil_homals,
         score_RM_R = Quantil_RM,
         score_TRI_R = Quantil_TRI) %>%  
  saveRDS(object = ., file = "Data/finaldata.rds")
